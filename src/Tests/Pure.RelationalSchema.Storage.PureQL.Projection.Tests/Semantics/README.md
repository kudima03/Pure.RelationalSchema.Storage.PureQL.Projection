...
| --- | --- | --- |
| Outer-join null extension | Unmatched side is padded with a fixed empty cell (`JoinApplicator.Pad`). Reading it back through `CellValueExtractor`: every typed column - `string` included, as of issue #167 - extracts as `null` (string used to extract as a real, non-null `""`, which wrongly let padded rows win string `min` and inflate `count`; see `Types/NullThreeValuedLogicTests.cs`). The raw *display* text of a padded cell (e.g. a plain, non-aggregate `SELECT` or `DISTINCT` on the padded column, which reads `ICell.TextValue` directly rather than through `CellValueExtractor`) is still the literal empty string `""`, matching this codebase's existing convention of rendering every NULL cell as `""` in projected output (see `GroupByApplicator`'s NULL group key and `DistinctApplicator`'s NULL dedup key, which do the same for every type, not just padding). Deterministic, never throws. | `JoinApplicator.cs` (`Pad`), `CellValueExtractor.cs` |
